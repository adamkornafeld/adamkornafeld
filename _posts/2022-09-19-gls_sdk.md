---
layout: article
title: DIY Software Development Kit
...
Today we are gonna implement an SDK in Python. The motivation being is that a close relative needed to integrate a web shop with the label printing service of GLS Parcel. GLS has a REST-like API, but they only publish sample code in a handful of languages that excludes Python. I thought this would be a perfect opportunity to demonstrate how to go from _zero to hero_ in a situation that a service provider publishes an [API](https://api.mygls.hu/index_en.html), but no SDK in your favorite language.
